...
// Which side advanced in a knockout match (won in regulation, extra time, or
// on penalties). NULL for group-stage matches and any match not yet decided.
export const advancerSide = pgEnum("advancer_side", ["HOME", "AWAY"]);
...
export const matches = pgTable(
  "matches",
  {
    ...
    // Knockout only: which side advanced (incl. extra time / penalties). The
    // score columns hold the legal-time (90'/120') result, which can be a draw;
    // `advancer` records who actually went through. NULL for group stage.
    advancer: advancerSide("advancer"),
